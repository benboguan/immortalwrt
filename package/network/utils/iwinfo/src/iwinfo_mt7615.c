	...
	if (mtk_ioctl(ifname, SIOCGIWSTATS, &wrq) >= 0)
	{
		if (is_5g(ifname)) {
			*buf = ((stats.qual.updated & IW_QUAL_DBM)
				? (stats.qual.noise - 0x100) : stats.qual.noise) - 36;
		} else {
			*buf = ((stats.qual.updated & IW_QUAL_DBM)
				? (stats.qual.noise - 0x100) : stats.qual.noise) - 30;
		}
...
static int mtk_get_quality(const char *ifname, int *buf)
{
	...
	if (!mtk_get_signal(ifname, &signal))
	{
		 ...
		 * by adding fix 110+20 to the mtk signal level */
		else
		{
			if (signal < -110)
				signal = -110;
	...
	*buf = 0;
	...
	if (!mtk_get_freqlist(ifname, chans, &len) )
	{
		for (e = (struct iwinfo_freqlist_entry *)chans; e->channel; e++ )
		{
			...
			{
				*buf = (IWINFO_HTMODE_HT20 | IWINFO_HTMODE_HT40 | IWINFO_HTMODE_VHT20 | IWINFO_HTMODE_VHT40
				| IWINFO_HTMODE_VHT80 | IWINFO_HTMODE_VHT80_80 | IWINFO_HTMODE_VHT160);
			}
		}
...
static int mtk_get_hardware_id_from_l1profile(struct iwinfo_hardware_id *id)
{
	...
	if (!strcmp(buf, "MT7615D") || !strcmp(buf, "MT7615")) {
		id->vendor_id = 0x14c3;
		id->device_id = 0x7615;
