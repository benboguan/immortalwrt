/*
 * switch_gsw.c: set for 7620 switch
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void usage(char *cmd)
{
	...
	printf(" %s del mac [mac] vid [vid]              - delete an entry from switch table\n", cmd);
	printf(" %s del mac [mac] fid [fid]		 - delete an entry from switch table\n", cmd);
	printf(" %s search mac [mac] vid [vid]		 - search an entry with specific mac and vid\n", cmd);
	printf(" %s search mac [mac] fid [fid]		 - search an entry with specific mac and fid\n", cmd);
	...
	printf(" %s igmpsnoop on [leaky_en]              - turn on IGMP snoop and  router port learning\n", cmd);
	printf("                                               leaky_en: 1 or 0. default 0.\n");
#endif
...
void acl_dip_meter(int argc, char *argv[])
{
	...
	printf("REG_VAWD1_ADDR value is 0x%x\n\r", value);

	value = j << 8;     //w_port_map
	value |= 0x1 << 19; //enable
	value |= 0x2 << 16; //ip header
	value |= 0x8 << 1;  //word offset
	reg_write(REG_VAWD2_ADDR, value);

	printf("REG_VAWD2_ADDR value is 0x%x\n\r", value);

	value = (0x80005000 + 0); //w_acl entry 0
	reg_write(REG_VTCR_ADDR, value);

	printf("REG_VTCR_ADDR value is 0x%x\n\r\n\r", value);
...
void igmp_on(int argc, char *argv[])
{
	...
	if (leaky_en == 1) {
		if (wan_num == 4) {
			...
			reg_write(REG_ESW_ISC, 0x10027d10);
		} else {
			...
			reg_read(0x2410, &value);
			reg_write(0x2410, value & (~(1 << 3)));
			reg_write(REG_ESW_ISC, 0x01027d01);
		}
	} else
		reg_write(REG_ESW_ISC, 0x10027d60);

	/* reg_write(REG_ESW_WT_MAC_MFC, 0xff00ff00); */
	reg_read(REG_ESW_WT_MAC_MFC, &value);
	reg_write(REG_ESW_WT_MAC_MFC, value & (~(0xff << 16)));
