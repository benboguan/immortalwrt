	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_CTRL_PLD to MESH_CTRL State-Machine.\n", __func__));
...
{
	RTMP_ADAPTER *pAd = (RTMP_ADAPTER *)FunctionContext;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_CTRL_MCS_TIMEOUT to MESH_CTRL State-Machine.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	*pCurrState = MESH_CTRL_ACTIVATED;
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Join evt when Idle.\n", __func__));
	AsicDisableSync(pAd);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MESH_CTRL_STATE *pCurrState = &(pAd->MeshTab.CtrlCurrentState);
	INT i;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Disconnect evt when Discovery.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MESH_CTRL_STATE *pCurrState = &(pAd->MeshTab.CtrlCurrentState);
	INT i;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Scan-Finish evt when Discovery.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	/* transite state to MESH_CTRL_ACTIVATED. */
#endif /* RELEASE_EXCLUDE */
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Scan-Abort evt when Discovery.\n", __func__));
	RTMPSetTimer(&pAd->MeshTab.PldTimer, PLD_TIME + RANDOM_TIME(pAd));
	*pCurrState = MESH_CTRL_ACTIVATED;
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MLME_SCAN_REQ_STRUCT ScanReq;
	UCHAR BroadSsid[MAX_LEN_OF_SSID];
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get PLD evt when Activated.\n", __func__));
	AsicDisableSync(pAd);
	BssTableInit(&pAd->ScanTab);
	BroadSsid[0] = '\0';
	ScanReq.SsidLen = 0;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	INT i;
	BOOLEAN Cancelled;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Disconnect evt when Activated.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	*pCurrState = MESH_CTRL_UCG;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get UCG evt when Activated.\n", __func__));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	INT i;
	BOOLEAN Cancelled;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Disconnect evt when UCG.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Mcs evt when UCG.\n", __func__));

	if (!INFRA_ON(pAd) && !ADHOC_ON(pAd)) {
		/*SET_MESH_CHANNEL(pAd); */
		wdev->channel = pAd->MeshTab.MeshChannel;
	...
	IN PMESH_CONFIGURAION_IE pMeshConfig)
{
	...
	if (pNeighborTab == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("pAd->MeshTab.pMeshNeighborTab equal NULL.\n"));
		return NULL;
	}
	...
	if (pNeighborEntry == NULL) {
		...
		if (pNeighborTab->NeighborNr >= MAX_NEIGHBOR_MP) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("pAd->MeshTab.pMeshNeighborTab FULL.\n"));
			return NULL;
		}
		...
		if (i < MAX_NEIGHBOR_MP) {
			...
			pNeighborEntry = NULL;
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Neighbor tab full.\n", __func__));
		}
	...
	IN PUCHAR pPeerMacAddr)
{
	...
	if (pNeighborTab->NeighborNr == 0) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("pMeshNeighborTab empty.\n"));
		return;
	}
		...
		if (RTMP_TIME_AFTER(Now, pNeighborEntry->LastBeaconTime + (MESH_NEIGHBOR_BEACON_IDLE_TIME * OS_HZ / 1000))) {
			...
			if (VALID_MESH_LINK_ID(pNeighborEntry->MeshLinkIdx)) {
				if ((pNeighborEntry->State == LINK_AVAILABLE)
					...
					MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Link%d:Neighbor ExtChOffset change from %d to %d , kill the link!\n"
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->MeshTab.CPI == 0)
		...
		if ((pNeighborEntry != NULL)
			...
			if (wdev->channel != pNeighborEntry->Channel) {
				pAd->MeshTab.CPI = pNeighborEntry->CPI;
		...
		for (i = 0; i < pNeighborTab->NeighborNr; i++) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("(%d) CPI=%d Apl=%d channel=%d state=%d\n", i, pNeighborEntry->CPI,
					 pNeighborEntry->MeshCapability.field.AcceptPeerLinks, pNeighborEntry->Channel, pNeighborEntry->State));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%d %d %d %d %d\n",
			...
			if ((GetMeshSecurity(pAd) == MeshCheckPeerMpCipher(pNeighborEntry->CapabilityInfo, pNeighborEntry->RSNIE, pNeighborEntry->RSNIE_Len))
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("(%d) pick LinkId=%ld\n", i, LinkIdx));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->MeshTab.pMeshNeighborTab)
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->MeshTab.pMeshNeighborTab", __func__));
...
{
	if (pAd->MeshTab.pMeshNeighborTab)
		os_free_mem(pAd->MeshTab.pMeshNeighborTab);
	...
	while (i < MAC_ADDR_LEN) {
		if (pAd->MeshTab.wdev.if_addr[i] != pAd->MeshTab.MeshLink[idx].Entry.PeerMacAddr[i]) {
			if (pAd->MeshTab.wdev.if_addr[i] > pAd->MeshTab.MeshLink[idx].Entry.PeerMacAddr[i]) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("The local MP is the Selector MP"));
			} else {
				pAd->MeshTab.MeshLink[idx].Entry.bValidLocalMpAsSelector = FALSE;
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("The local MP is NOT the Selector MP"));
			}
