...
INT Set_MeshId_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (strlen(arg) <= MAX_MESH_ID_LEN) {
		...
		success = TRUE;
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) Set_MeshId_Proc::(Len=%d,MeshId=%s)\n",
...
INT Set_MeshHostName_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (strlen(arg) <= MAX_HOST_NAME_LEN) {
		...
		success = TRUE;
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) Set_MeshHostName_Proc::(HostName=%s)\n",
...
INT Set_MeshAutoLink_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(enable = %d)\n", __func__, pAd->MeshTab.MeshAutoLink));
	return TRUE;
}
...
INT Set_MeshForward_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(enable = %d)\n", __func__, pAd->MeshTab.MeshCapability.field.Forwarding));
	return TRUE;
}
...
INT Set_MeshAddLink_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (LinkIdx == BSS_NOT_FOUND) {
		...
		if (LinkIdx == BSS_NOT_FOUND) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() All Mesh-Links been occupied.\n", __func__));
			return FALSE;
		}
	}
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(LinkIdx = %ld)\n", __func__, LinkIdx));
	return TRUE;
}
...
INT Set_MeshMaxTxRate_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	Rate = os_str_tol(arg, 0, 10);

	if (Rate <= 12)
		pAd->MeshTab.MeshMaxTxRate = Rate;
	else
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s::Wrong Tx Rate setting(%d), (0 ~ 12))\n", __func__, Rate));

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(Max Tx Rate = %ld)\n", __func__, pAd->MeshTab.MeshMaxTxRate));
	return TRUE;
}
...
INT Set_MeshRouteAdd_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		if (Count == 0) {
			...
			if ((thisChar != NULL) && (strlen(thisChar) > 0))
				Metric = os_str_tol(thisChar, 0, 10);
			else
				return FALSE;
		} else
			return FALSE;
	...
	if (MeshEntryTableLookUp(pAd, DestAddr)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("The Dest Mesh Point aleady on Route Table!!!\n"));
		return FALSE;
	} else {
		pMacEntry = MeshTableLookup(pAd, NextHop, TRUE);

		if (pMacEntry == NULL) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Next Hop have not on Mac Table !!!\n"));
			return FALSE;
		}
		...
		if (!(pEntry && MAC_ADDR_EQUAL(pEntry->NextHop, NextHop))) {
			...
			if (!pEntry) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Add Dest Mesh Point Route fail !!!\n"));
				return FALSE;
			}
		}
...
INT Set_MeshRouteUpdate_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pMeshEntry) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The can not find the Mesh Route !!!\n"));
		return FALSE;
	}
...
INT Set_MeshMultiCastAgeOut_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	AgeTime = os_str_tol(arg, 0, 10);

	if ((AgeTime <= 65535) && (AgeTime >= 1))
		pAd->MeshTab.MeshMultiCastAgeOut = AgeTime;
	else
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s::Wrong MeshMultiCastAgeOut setting(%d), (1 ~ 65535))\n", __func__, AgeTime));

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(MeshMultiCastAgeOut = %ld)\n", __func__, pAd->MeshTab.MeshMultiCastAgeOut));
...
INT Set_MeshAuthMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) Set_MeshAuthMode_Proc::(MeshAuthMode(%d)=%s)\n",
...
INT Set_MeshEncrypType_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	RTMPMakeRSNIE(pAd, wdev->AuthMode, wdev->WepStatus, MIN_NET_DEVICE_FOR_MESH);
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) Set_MeshEncrypType_Proc::(MeshEncrypType(%d)=%s)\n",
...
INT Set_MeshDefaultkey_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) Set_MeshDefaultkey_Proc::(MeshDefaultkey=%d)\n",
			 wdev->DefaultKeyId));
	return success;
}
...
INT Set_MeshWEPKEY_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	case 13: /*wep 104 Ascii type */
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("IF(mesh0) Set_MeshWEPKRY_Proc::(WepKey=%s ,type=%s, Alg=%s)\n", arg, "Ascii", CipherName[CipherAlg]));
		break;
	...
	default: /*Invalid argument */
		...
		pAd->MeshTab.DesiredWepKeyLen = KeyLen;
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("IF(mesh0) Set_MeshWEPKRY_Proc::Invalid argument (=%s)\n", arg));
		return FALSE;
	}
...
INT Set_MeshWPAKEY_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	success = SetWPAPSKKey(pAd, arg, strlen(arg), pAd->MeshTab.MeshId, pAd->MeshTab.MeshIdLen, pAd->MeshTab.PMK);
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("IF(mesh0) Set_MeshWPAKEY_Proc::PassPhrasKey (=%s)\n", arg));
	return success;
}
...
INT Set_MeshRouteInfo_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pRouteTab == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Route Table doesn't exist.\n", __func__));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("\n\n"));
	...
	for (i = 0; i < MAX_ROUTE_TAB_SIZE; i++) {
		...
		if (pEntry->Valid) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02X:%02X:%02X:%02X:%02X:%02X  ", PRINT_MAC(pEntry->MeshDA)));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", pEntry->Dsn));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02X:%02X:%02X:%02X:%02X:%02X  ", PRINT_MAC(pEntry->NextHop)));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-16d", (int)pEntry->NextHopLinkID));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-12d", pEntry->PathMetric));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-9d\n", (int)pEntry->Idx));
		}
	}
...
INT Set_MeshProxyInfo_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	int i = 0;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("pProxyTab size:%d free:%d\n", pProxyTab->AgeList.Number, pProxyTab->FreeEntryList.Number));
	EntryIndex = pProxyTab->AgeList.Head;

	if (EntryIndex >= 0)
		pEntry =  &pProxyTab->Content[EntryIndex];

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh Proxy Age Out List\n"));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Index,DLNext,DLPrev,HashNext,Mac\n"));

	while (EntryIndex !=  -1) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02d %02d %02d %02d MAC:%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Mesh Proxy Mac Hash Table\n"));

	for (i = 0; i < MAX_HASH_ENTRY_TAB_SIZE; i++) {
		if (pProxyTab->Hash[i] !=  -1)
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%03d : %d\n", i, pProxyTab->Hash[i]));
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh Content Table\n"));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("No,HashNext,DLNext,DLPrev\n"));

	for (i = 0; i < MAX_HASH_PROXY_ENTRY_TAB_SIZE; i++)
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" %d: %02d %02d %02d\n", i, pProxyTab->Content[i].Next, pProxyTab->Content[i].DLNext, pProxyTab->Content[i].DLPrev));
...
INT Set_MeshInfo_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	wdev = &pAd->MeshTab.wdev;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("HostName = %s, Len=%d\n", pAd->MeshTab.HostName, strlen((RTMP_STRING *)pAd->MeshTab.HostName)));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh Id = %s, Len=%d\n", pAd->MeshTab.MeshId, pAd->MeshTab.MeshIdLen));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh AutoLink = %s\n", pAd->MeshTab.MeshAutoLink == TRUE ? "Enable" : "Disable"));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Channel Precedence (CPI) = %d\n", pAd->MeshTab.CPI));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("mesh ctrl current state =%d\n", pAd->MeshTab.CtrlCurrentState));
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh AuthMode(%d)=%s, EncrypType(%d)=%s\n",
			 wdev->AuthMode, GetAuthMode(wdev->AuthMode),
			 wdev->WepStatus, GetEncryptType(wdev->WepStatus)));

	for (i = 0; i < MAX_MESH_LINKS; i++) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("mesh link (%d) current state =%d,", i, pAd->MeshTab.MeshLink[i].CurrentState));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Valid =%d,", pAd->MeshTab.MeshLink[i].Entry.Valid));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" MatchWcid =%d,", pAd->MeshTab.MeshLink[i].Entry.MacTabMatchWCID));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" LocalId =%x,", pAd->MeshTab.MeshLink[i].Entry.LocalLinkId));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" PeerId =%x,", pAd->MeshTab.MeshLink[i].Entry.PeerLinkId));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" ExtChOffset =%d,", pAd->MeshTab.MeshLink[i].Entry.ExtChOffset));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" PeerMacAddr =%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (IS_ENTRY_MESH(pEntry)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02X:%02X:%02X:%02X:%02X:%02X  ",
					 pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
					 pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5]));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", (int)pEntry->func_tb_idx));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", (int)pEntry->Aid));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", (int)pEntry->PsMode));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[0]));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[1]));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[2]));
#ifdef DOT11_N_SUPPORT
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-10s", get_phymode_str(pEntry->HTPhyMode.field.MODE)));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6s", get_bw_str(pEntry->HTPhyMode.field.BW)));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.MCS));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.ShortGI));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d\n", pEntry->HTPhyMode.field.STBC));
...
INT Set_NeighborInfo_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pAd->MeshTab.pMeshNeighborTab == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh Neighbor Tab not ready.\n"));
		return TRUE;
	}

	if (pAd->MeshTab.pMeshNeighborTab->NeighborNr == 0) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Mesh Neighbor Tab empty.\n"));
		return TRUE;
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Neighbor MP Num = %d\n", pAd->MeshTab.pMeshNeighborTab->NeighborNr));
#if 0
	...
	for (i = 0; i < MAX_NEIGHBOR_MP; i++) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%d, HostName=%s,", i, pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].HostName));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Rssi=%d,", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].RealRssi));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Ch=%d,", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].Channel));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" CPI=%d,", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].CPI));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" State=%d,", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].State));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" LinkId=%d,", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshLinkIdx));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" MeshId=%s, len=%d", pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshId,
				 pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshIdLen));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Mac=%02x:%02x:%02x:%02x:%02x:%02x,",
		...
		if (pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshEncrypType == ENCRYPT_OPEN_WEP)
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Encryption=OPEN-WEP\n"));
		else if (pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshEncrypType == ENCRYPT_WPANONE_TKIP)
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Encryption=WPANONE-TKIP\n"));
		else if (pAd->MeshTab.pMeshNeighborTab->NeighborMP[i].MeshEncrypType == ENCRYPT_WPANONE_AES)
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Encryption=WPANONE-AES\n"));
		else
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" Encryption=OPEN-NONE\n"));
	}
	...
	for (i = 0; i < MAX_NEIGHBOR_MP; i++) {
		...
		if (pEntry->Valid) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", i));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02X:%02X:%02X:%02X:%02X:%02X  ",
					 pEntry->PeerMac[0], pEntry->PeerMac[1], pEntry->PeerMac[2],
					 pEntry->PeerMac[3], pEntry->PeerMac[4], pEntry->PeerMac[5]));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", (int)pEntry->RealRssi));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", (int)pEntry->Channel));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-4d", (int)pEntry->ChBW));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", (int)pEntry->ExtChOffset));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-8d", (int)pEntry->CPI));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-6d", (int)pEntry->State));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-8x", (int)pEntry->MeshLinkIdx));
			...
			if (MeshEncrypType == ENCRYPT_OPEN_WEP)
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-14s", "OPEN-WEP"));
			else if (MeshEncrypType == ENCRYPT_WPANONE_TKIP)
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-14s", "WPANONE-TKIP"));
			else if (MeshEncrypType == ENCRYPT_WPANONE_AES)
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-14s", "WPANONE-AES"));
			else
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-14s", "OPEN-NONE"));

			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-16s", pEntry->MeshId));
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%s\n", pEntry->HostName));
		}
	}
...
INT Set_MultipathInfo_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MAX_MESH_LINKS; i++) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Link(%d) ", i));
		...
		for (HashId = 0; HashId < MULTIPATH_HASH_TAB_SIZE; HashId++) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, (" HashId(%ld):", HashId));

			while (pEntry) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("SA=%02x:%02x:%02x:%02x:%02x:%02x ",
...
INT Set_MultiCastAgeOut_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("Multi Cast Age Timeout = %ld sec\n", (pAd->MeshTab.MeshMultiCastAgeOut) / 1000));
	return TRUE;
}
...
INT Set_PktSig_Display_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MAX_BMPKTSIG_TAB_SIZE; i++) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%02x:%02x:%02x:%02x:%02x:%02x  ",
				 PRINT_MAC(pEntry->MeshSA)));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-8x:%-9x%-9x%-9x%-9x",
	...
	IN PRTMP_ADAPTER pAd)
{
	struct wifi_dev *wdev;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("%s --->\n", __func__));
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: ===>\n", __func__));

	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS))
		/*||!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_START_UP)) */
		return;

	pAd->MeshTab.MeshChannel = wdev->channel;
	...
	if (pAd->MeshTab.EasyMeshSecurity) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshUp: the Easy MSA is enabled.\n"));
	} else {
#ifdef RELEASE_EXCLUDE
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshUp: the Easy MSA is disabled.\n"));
	}
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		/* TODO: Hanmin, need further check WCID */
		AsicBssInfoUpdate(
			pAd,
			wdev->OmacIdx,
	...
	if (wdev->AuthMode == Ndis802_11AuthModeWPANone) {
		...
		if (wdev->WepStatus == Ndis802_11TKIPEnable)
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_WARN, ("Unknow Cipher (=%d), set Cipher to AES\n", wdev->WepStatus));
			pAd->MeshTab.SharedKey.CipherAlg = CIPHER_AES;
		}
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("!!! %s - AuthMode(%d)=%s, WepStatus(%d)=%s !!!\n",
			 __func__,
			 wdev->AuthMode, GetAuthMode(wdev->AuthMode),
			 wdev->WepStatus, GetEncryptType(wdev->WepStatus)));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: tear down all Mesh Link.\n", __func__));
			...
			*/
			/*send link close message */
			{
				...
				if (NStatus != NDIS_STATUS_SUCCESS) {
					MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() allocate memory failed\n", __func__));
					return;
				}
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId=%x, PeerLinkId=%x, Reason=%d\n",
						 __func__, LocalLinkId, PeerLinkId, MESH_LINK_CANCELLED));
			}
	...
	IN  UINT MeshLinkIdx)
{
	...
	do {
		pEntry = MeshTableLookup(pAd, pAddr, TRUE);
		if (pEntry != NULL)
			break;
		...
		if (pEntry) {
			...
			if (wdev->HTPhyMode.field.MODE >= MODE_HTMIX) {
				if (wdev->DesiredTransmitSetting.field.MCS != MCS_AUTO) {
					MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("IF-mesh0 : Desired MCS = %d\n",
...
#endif /* DOT11_N_SUPPORT */
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MacTableInsertMeshEntry - allocate entry #%d, Total= %d\n", pEntry->wcid, pAd->MacTab.Size));
			break;
		}
	} while (FALSE);
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> CmmRxMeshMgmtFrameHandle\n"));
#endif /* RELEASE_EXCLUDE */
#ifdef WDS_SUPPORT

	if (WdsTableLookup(pAd, pHeader->Addr2, FALSE) != NULL) {
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("%s() Peer WDS entry.\n", __func__));
#endif /* RELEASE_EXCLUDE */
		return;
	}
	...
	IN	PUCHAR	pAddr)
{
	...
	UCHAR	DestAddr[MAC_ADDR_LEN];
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> PathRouteIDSearch\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("%s ---> PathReqTimerRunning is TRUE\n", __func__));
#endif /* RELEASE_EXCLUDE */
			return -1;
		} else
	...
	IN	UCHAR	RouteIdx)
{
	...
	if (pRouteTab->Content[RouteIdx].Valid == TRUE)
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Can't find the Route Index = (%d)\n", (int)RouteIdx));
		return NULL;
	}
	...
	IN PUCHAR pSrcBufVA)
{
	...
	default:
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Undown Mesh AE type=%x\n", __func__, GetMeshFlagAE(pSrcBufVA)));
		break;
	}
		...
		*pbDirectForward = FALSE;
		...
		if (*pbAnnounce == TRUE) {
			...
			if (MeshFlagAE == 2) {
				...
				/* drop the frame. */
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Receive Mesh-Data frame carry AE=1. Drop the frame.\n", __func__));
				*pbAnnounce = FALSE;
				*pbDirectForward = FALSE;
	...
	RTMP_STRING *buffer)
{
	...
	if (RTMPGetKeyParameter("MeshId", tmpbuf, 255, buffer, FALSE)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshIdLen=%d, MeshId=%s\n", pAd->MeshTab.MeshIdLen, pAd->MeshTab.MeshId));
	}

	/*MeshAutoLink */
	if (RTMPGetKeyParameter("MeshAutoLink", tmpbuf, 255, buffer, TRUE)) {
		LONG Enable;
		Enable = os_str_tol(tmpbuf, 0, 10);
		pAd->MeshTab.MeshAutoLink = (Enable > 0) ? TRUE : FALSE;
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(MeshAutoLink=%d)\n", __func__, pAd->MeshTab.MeshAutoLink));
	} else
	...
	if (RTMPGetKeyParameter("MeshForward", tmpbuf, 255, buffer, TRUE)) {
		...
		Enable = os_str_tol(tmpbuf, 0, 10);
		pAd->MeshTab.MeshCapability.field.Forwarding = (Enable > 0) ? (1) : (0);
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s::(MeshForward=%d)\n", __func__, pAd->MeshTab.MeshCapability.field.Forwarding));
	}
	...
	if (RTMPGetKeyParameter("MeshAuthMode", tmpbuf, 255, buffer, TRUE)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) MeshAuthMode(%d)=%s\n", wdev->AuthMode, GetAuthMode(wdev->AuthMode)));
		RTMPMakeRSNIE(pAd, wdev->AuthMode, wdev->WepStatus, MIN_NET_DEVICE_FOR_MESH);
	...
	if (RTMPGetKeyParameter("MeshEncrypType", tmpbuf, 255, buffer, TRUE)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) MeshEncrypType(%d)=%s\n", wdev->WepStatus, GetEncryptType(wdev->WepStatus)));
		RTMPMakeRSNIE(pAd, wdev->AuthMode, wdev->WepStatus, MIN_NET_DEVICE_FOR_MESH);
	}
	...
	if (RTMPGetKeyParameter("MeshWPAKEY", tmpbuf, 255, buffer, TRUE)) {
		...
		int success;
		success = SetWPAPSKKey(pAd, tmpbuf, strlen(tmpbuf), pAd->MeshTab.MeshId, pAd->MeshTab.MeshIdLen, pAd->MeshTab.PMK);

		if (success == TRUE) {
			NdisMoveMemory(pAd->MeshTab.WPAPassPhraseKey, tmpbuf, strlen(tmpbuf));
			pAd->MeshTab.WPAPassPhraseKeyLen = strlen(tmpbuf);
		} else
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("I/F(mesh0) set MeshWPAKEY fail, key string required 8 ~ 64 characters!!!\n"));
	}
	...
	if (RTMPGetKeyParameter("MeshDefaultkey", tmpbuf, 255, buffer, TRUE)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) DefaultKeyID(0~3)=%d\n", wdev->DefaultKeyId));
	}
	...
	if (RTMPGetKeyParameter("MeshWEPKEY", tmpbuf, 255, buffer, TRUE)) {
		...
		if ((KeyLen == 10) || (KeyLen == 26)) {
			...
			pAd->MeshTab.DesiredWepKeyLen = KeyLen;
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) MeshWEPKEY=%s, it's HEX type and %s\n", tmpbuf, (KeyLen == 10) ? "wep64" : "wep128"));
		}
		/* ASCII type */
			...
			NdisMoveMemory(pAd->MeshTab.DesiredWepKey, tmpbuf, KeyLen);
			pAd->MeshTab.DesiredWepKeyLen = KeyLen;
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("I/F(mesh0) MeshWEPKEY=%s, it's ASCII type and %s\n", tmpbuf, (KeyLen == 5) ? "wep64" : "wep128"));
		}
		/*Invalid key length */
		else {
			pAd->MeshTab.DesiredWepKeyLen = 0;
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("I/F(mesh0) MeshWEPKEY is Invalid key length(%d)!\n", (UCHAR)KeyLen));
		}
	}
	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	struct wifi_dev *wdev;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("RTMPIoctlQueryMeshSecurityInfo==>\n"));
	...
	if (copy_to_user(wrq->u.data.pointer, &meshInfo, wrq->u.data.length))
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: copy_to_user() fail\n", __func__));
}
	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	UCHAR	MeshKey[255];
	struct wifi_dev *wdev;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("RTMPIoctlSetMeshSecurityInfo==>\n"));
	wdev = &pAd->MeshTab.wdev;

	if (wrq->u.data.length != sizeof(MESH_SECURITY_INFO)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: the length is too large\n", __func__));
		return -EINVAL;
	}

	if (copy_from_user(&meshInfo, wrq->u.data.pointer, wrq->u.data.length)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: copy_from_user() fail\n", __func__));
		return -EFAULT;
	}
	...
	if (meshInfo.EncrypType == ENCRYPT_OPEN_WEP) {
		if (!Set_MeshWEPKEY_Proc(pAd, (RTMP_STRING *) MeshKey)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh WEP key isn't valid\n", __func__));
			return -EFAULT;
		}
		...
		if (!Set_MeshWPAKEY_Proc(pAd, (RTMP_STRING *) MeshKey)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh WPA key isn't valid\n", __func__));
			return -EFAULT;
		}
...
UCHAR GetMeshSecurity(RTMP_ADAPTER *pAd)
{
	...
	if (wdev->AuthMode == Ndis802_11AuthModeOpen
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR,
				 ("%s: Can not find correct mesh security ,return OPEN_NONE Auth:%d WepStatus:%d\n",
				  __func__, wdev->AuthMode, wdev->WepStatus));
	}
	...
	do {
		...
		if (pMesh->wdev.if_dev == pDev) {
			...
			if (!(*(GET_OS_PKT_DATAPTR(pPktSrc)) & 0x01)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO,
	...
	OUT PUCHAR *ppMeshAddr6)
{
	...
	PHEADER_802_11 pHeader_802_11 = (PHEADER_802_11)&pTxBlk->HeaderBuf[TXINFO_SIZE + TXWISize];
	...
	IN PMESH_NEIGHBOR_ENTRY pNeighborEntry)
{
	...
	struct wifi_dev *wdev = &pAd->MeshTab.wdev;
	CHAR ext_cha = wlan_operate_get_ext_cha(wdev);
	...
	if (WMODE_CAP_N(wdev->PhyMode)
		...
		result = ((pNeighborEntry->Channel == wdev->channel)
				  && (pNeighborEntry->ChBW == op_ht_bw)
				  && (pNeighborEntry->ExtChOffset == ext_cha)) ? TRUE : FALSE;
	} else
#endif /* DOT11_N_SUPPORT */
	{
		result = (pNeighborEntry->Channel == wdev->channel) ? TRUE : FALSE;
	}
	...
	IN UCHAR			idx)
{
	...
	RTUSBBssBeaconStop(pAd);
#endif /* RTMP_MAC_USB */
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshMakeBeacon - %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	/* AP Channel Report */
	if (wdev->PhyMode == (WMODE_B | WMODE_G | WMODE_GN)
...
#endif
	mac_info.SpeEn = 1;
	pAd->archOps.write_tmac_info_fixed_rate(pAd, tmac_info, &mac_info, &BeaconTransmit);
	/*
...
#endif
	mac_info.SpeEn = 1;
	pAd->archOps.write_tmac_info_fixed_rate(pAd, (UCHAR *)&pAd->BeaconTxWI, &mac_info, &BeaconTransmit);
	/*
		step 7. move BEACON TXWI and frame content to on-chip memory
	*/
	MtUpdateBeaconToAsic(pAd, idx + MIN_NET_DEVICE_FOR_MESH, FrameLen);
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* TODO: shiang-7603 */
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%s(): Not support for HIF_MT yet!\n",
				 __func__));
		return FALSE;
	}
	...
	IN UINT32 Metric)
{
	...
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Dest address(%02x:%02x:%02x:%02x:%02x:%02x)\n",
			 __func__, pDestAddr[0], pDestAddr[1], pDestAddr[2],
	...
	IN RX_BLK *pRxBlk)
{
	...
	do {
		...
		if (Idx == BSS_NOT_FOUND) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() Link Metric Report Ignore, PeerLink Not found.\n", __func__));
			return;
		}
		...
		MeshLinkMetricReportSanity(pAd,
								   ...
								   FrameLen,
								   &LinkMetric);
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LinkMetric=%d, PeerMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				 __func__, LinkMetric, pHeader->Addr2[0], pHeader->Addr2[1],
		...
		if (RouteId == BMCAST_ROUTE_ID) {
			...
			if ((LinkId >= 0) && VALID_MESH_LINK_ID(LinkId)) {
				...
				send_data_pkt(pAd, wdev, pForwardPacket);
					...
					MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): tx_pkt_handle not assigned!\n",
							 __func__));
					RELEASE_NDIS_PACKET(pAd, pForwardPacket, NDIS_STATUS_FAILURE);
				}
			} else
...
static struct wifi_dev_ops mesh_wdev_ops = {
	.tx_pkt_allowed = mesh_tx_pkt_allowed,
	.fp_tx_pkt_allowed = mesh_tx_pkt_allowed,
	.send_data_pkt = ap_send_data_pkt,
	.send_fp_data_pkt = ap_fp_send_data_pkt,
	...
	.rx_pkt_foward = mesh_rx_foward_handle,
...
#define MESH_MAX_DEV_NUM	32
	...
	do {
		...
		if (new_dev_p == NULL) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("Allocate network device fail (MESH)...\n"));
			break;
		}

		Ret = wdev_init(pAd, wdev, WDEV_TYPE_MESH, new_dev_p, 0, &pAd->MeshTab, (VOID *)pAd);

		if (!Ret) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("Assign wdev idx for %s failed, free net device!\n",
					 RTMP_OS_NETDEV_GET_DEVNAME(new_dev_p)));
			RtmpOSNetDevFree(new_dev_p);
			break;
		}

		Ret = wdev_ops_register(wdev, WDEV_TYPE_MESH, &mesh_wdev_ops,
