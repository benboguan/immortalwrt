	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->MeshTab.pMeshEntryTab)
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->MeshTab.pMeshEntryTab", __func__));
	...
	IN PUCHAR			DestAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshEntryTableLookUp\n"));
#endif /* RELEASE_EXCLUDE */

	if (pEntryTab == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Entry Table doesn't exist.\n", __func__));
		return NULL;
	}
	...
	IN UCHAR			Idx)
{
	...
	PMESH_ENTRY_TABLE pEntryTab = pAd->MeshTab.pMeshEntryTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshEntryTableInsert\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (pEntryTab->Size > MAX_HASH_ENTRY_TAB_SIZE) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Entry Table size more than 256 .\n", __func__));
		return NULL;
	}
	...
	IN PUCHAR		DestAddr)
{
	...
	PMESH_ENTRY_TABLE pEntryTab = pAd->MeshTab.pMeshEntryTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshEntryTableDelete\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN UCHAR			Idx)
{
	...
	PMESH_ENTRY_TABLE pEntryTab = pAd->MeshTab.pMeshEntryTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshEntryTableUpdate\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	for (i = 0; i < HASH_TABLE_SIZE; i++) {
		...
		while (pEntry != NULL) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%-10d\n", (int)pEntry->Idx));
			pEntry = pEntry->pNext;
		}
	}
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	if (pAd->MeshTab.pMeshProxyTab) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->MeshTab.pMeshProxyTab", __func__));
	...
	IN PRTMP_ADAPTER	pAd)
{
	if (pAd->MeshTab.pMeshProxyTab == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Proxy Table doesn't exist.\n", __func__));
		return TRUE;
	}
	...
	IN PUCHAR			pSA)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshProxyEntryTableLookUp\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (!(pAd->MeshTab.OpMode & MESH_AP) &&  !(pAd->MeshTab.OpMode & MESH_POTAL)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Proxy does not enable!\n", __func__));
		return NULL;
	}
	...
	IN PUCHAR			pSA)
{
	...
	PMESH_PROXY_ENTRY_TABLE	pProxyTab = pAd->MeshTab.pMeshProxyTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshProxyEntryTableInsert\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (GetMeshLinkId(pAd, (PCHAR)pSA) != BSS_NOT_FOUND) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: The address are neighbor peer mesh link .\n", __func__));
		return NULL;
	}

	if (MAC_ADDR_EQUAL(ZERO_MAC_ADDR, pSA)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Receive Zero MAC Address .\n", __func__));
		return NULL;
	}
	...
	if (NewEntryIndex == -1) {
		...
		if (NewEntryIndex == -1) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Bug when remove doublelinkheadqueue .\n", __func__));
			RTMP_SEM_UNLOCK(&pAd->MeshTab.MeshProxyTabLock);
			return NULL;
		}
		...
		if (RTMP_TIME_AFTER(Now, pProxyTab->Content[NewEntryIndex].LastUpdateTime + MESH_PROXY_AGEOUT))
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("Proxy table is full , Age Out Proxy Entry: Index:%d HashIdx:%d\n", NewEntryIndex, HashIdx));
		else
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Age Out Expired Proxy Entry: Index:%d HashIdx:%d\n", NewEntryIndex, HashIdx));
		...
		if (CurrEntryIndex == NewEntryIndex) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("BUG in MeshProxyEntry Table %d\n", CurrEntryIndex));
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Proxy Entry = (%02x:%02x:%02x:%02x:%02x:%02x) !!!\n",
	...
	IN PUCHAR			pSA)
{
	...
	PMESH_PROXY_ENTRY_TABLE	pProxyTab = pAd->MeshTab.pMeshProxyTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshProxyEntryTableDelete\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN PUCHAR			pSA)
{
	...
	PMESH_PROXY_ENTRY_TABLE	pProxyTab = pAd->MeshTab.pMeshProxyTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshProxyEntryTableUpdate\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	if (pAd->MeshTab.pMeshRouteTab)
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->MeshTab.pMeshRouteTab", __func__));
	...
	IN  PUCHAR			MeshDA)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("-----> MeshRoutingTableLookup\n"));
	...
	IN	UINT32			Metric)
{
	...
	PMESH_ROUTING_TABLE	pRouteTab = pAd->MeshTab.pMeshRouteTab;
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("-----> MeshRoutingTableInsert\n"));
	...
	if (pRouteTab->Size >= MAX_ROUTE_TAB_SIZE) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Mesh Route Table size > %d.\n", __func__, MAX_ROUTE_TAB_SIZE));
		return NULL;
	}
	...
	IN PUCHAR MeshDA)
{
	...
	PMESH_ENTRY_TABLE pEntryTab = pAd->MeshTab.pMeshEntryTab;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshRoutingTableDelete\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (i == MAX_ROUTE_TAB_SIZE) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: The Mesh Route  Entry doesn't exist.\n", __func__));
		RTMP_SEM_UNLOCK(&pAd->MeshTab.MeshRouteTabLock);
		return FALSE;
	}
			...
			do {
				if (pEntry->Idx == pMeshProbeEntry->Idx) {
					if (pMeshProbeEntry->PathReq) {
						...
						os_free_mem(pMeshProbeEntry->PathReq);
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshRoutingTableUpdate\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN	PUCHAR			Precursor)
{
	MESH_ROUTING_ENTRY *pEntry = NULL;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("-----> MeshRoutingTablePrecursorUpdate\n"));
#endif /* RELEASE_EXCLUDE */

	pEntry = MeshRoutingTableLookup(pAd, MeshDestAddr);
	if (pEntry == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("Can't find the route entry !\n"));
		return NULL;
	}
