	...
	IN UCHAR wcid)
{
	...
	if (MWDSConnEntryLookupByWCID(pAd, ifIndex, wcid))
		return;
			...
			if (pConnEntry && pConnEntry->Valid && VALID_WCID(pConnEntry->wcid)) {
				...
				if ((wdev->enable_easy_setup)
					...
					if (drop_pkt_to_dup_ez_apcli && pEntry->link_duplicate) { /* todo: fill it based on ez_peer_securtity cli_duplicate info */
						/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("MWDSSendClonePacket: Skip sending Group packet to my CLI as it's other band CLI connected to my other AP\n"));*/
						continue;
					} else {
				...
				MWDSHardTransmit(pAd, wdev, pPacketClone);
			}
		}
	}
}
	...
	IN PMAC_TABLE_ENTRY pEntry)
{
	...
	if (
		...
		/*
		    1. For this moment, we can directly set up ToDS/FromDS for WTBL by FW command after port secured.
		    2. For TKIP+4-addr ping fail issue, we need to assign "ucDisRhtr" to zero for HW workaround issue.
		*/
#endif
