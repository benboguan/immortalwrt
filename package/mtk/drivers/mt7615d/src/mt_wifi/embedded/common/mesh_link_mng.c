	...
	IN USHORT Idx)
{
	...
	if (!VALID_MESH_LINK_ID(Idx)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Invalid Link Index (%d).\n", __func__, Idx));
		return;
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("!!! Mesh link-%d is established !!!\n", Idx));

	/* Check if the current state is established. */
	if (pAd->MeshTab.MeshLink[Idx].CurrentState != MESH_LINK_MNG_ESTAB) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("!!! ERROR : This link(%d) isn't established (state=%d)!!!\n", Idx, pAd->MeshTab.MeshLink[Idx].CurrentState));
		return;
	}
	...
	pEntry = MeshTableLookup(pAd, pAd->MeshTab.MeshLink[Idx].Entry.PeerMacAddr, TRUE);
	if (pEntry == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("!!! ERROR : This link(%d) doesn't exist in MAC_TABLE_ENTRY. (%02x:%02x:%02x:%02x:%02x:%02x)!!!\n",
	...
	do {
		...
		for (i = 0; i < MAX_MESH_LINKS; i++) {
			if (pAd->MeshTab.MeshLink[i].Entry.Valid == FALSE) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId(%d)=%x\n", __func__, i, pAd->MeshTab.MeshLink[i].Entry.LocalLinkId));
				break;
			}
		}

		if (i == MAX_MESH_LINKS) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Unable to allocate Mesh-Link.\n", __func__));
	...
	IN PVOID SystemSpecific3)
{
	...
	if (idx == MAX_MESH_LINKS) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - unknow timer.\n", __func__));
		return;
	}

	if (pAd->MeshTab.MeshLink[idx].Entry.OpenRetyCnt < MAX_OPEN_RETRY) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_LINK_MNG_TOR1 to MESH_LINK_MNG_STATE_MACHINE State-Machine. idx(%d)\n", __func__, idx));
		MlmeEnqueue(pAd, MESH_LINK_MNG_STATE_MACHINE, MESH_LINK_MNG_TOR1, 0, NULL, idx);
	} else {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_LINK_MNG_TOR2 to MESH_LINK_MNG_STATE_MACHINE State-Machine.idx(%d)\n", __func__, idx));
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_LINK_MNG_TOC to MESH_LINK_MNG_STATE_MACHINE State-Machine. idx(%d)\n", __func__, idx));
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s - enqueue MESH_LINK_MNG_TOH to MESH_LINK_MNG_STATE_MACHINE State-Machine. idx(%d)\n", __func__, idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!VALID_MESH_LINK_ID(Idx)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Invalid Link-Idx=%d", __func__, Idx));
		return;
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get PASOPN Event at Idle (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get ACTOPN Event at Idle (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CLSAcpt event at Listen (Idx=%d).\n", __func__, Idx));
	*pCurrState = MESH_LINK_MNG_LISTEN;
	return;
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get MlmeCNCL enent at Listen (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNAcpt at Listen (Idx=%d).\n", __func__, Idx));
	...
	if (pMacEntry) {
		...
		*pCurrState = MESH_LINK_MNG_OPN_RCVD;
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Unable to allocate MacEntry here.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOR1 event at OpnSnt (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkOpen(pAd, Idx);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOR, TOR_TIME);
	return;
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNAcpt event at OpnSnt (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNAcpt event at OpnSnt (Idx=%d).\n", __func__, Idx));
	pAd->MeshTab.MeshLink[Idx].Entry.PeerLinkId = pInfo->PeerLinkId;
	pMacEntry = MacTableInsertMeshEntry(pAd, pAd->MeshTab.MeshLink[Idx].Entry.PeerMacAddr, Idx);
	...
	if (pMacEntry) {
		...
		*pCurrState = MESH_LINK_MNG_OPN_RCVD;
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: Unable to a MacEntry here.\n", __func__));
		EnquePeerLinkConfirm(pAd, Idx, 0, MLME_FAIL_NO_RESOURCE);
		RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
		*pCurrState = MESH_LINK_MNG_HOLDING;
	}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get ClsAcpt event at OpnSnt (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNRjct event at OpnSnt (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNRjct event at OpnSnt (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOR2 event at OpnSnt (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Cncl event at OpnSnt (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNAcpt event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CLSAcpt event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOC, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, MESH_CLOSE_RCVD);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNRjct event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOC, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNRjct event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOC, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Cncl event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOC, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, MESH_LINK_CANCELLED);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOC event at CfnRcvd (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOR1 event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkOpen(pAd, Idx);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOR, TOR_TIME);
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNAcpt event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	pMacEntry = MeshTableLookup(pAd, pAd->MeshTab.MeshLink[Idx].Entry.PeerMacAddr, TRUE);

	if (pMacEntry)
		EnquePeerLinkConfirm(pAd, Idx, pMacEntry->Aid, MLME_SUCCESS);
	else
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("%s: MacEntry doesn't exist.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNAcpt event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	*pCurrState = MESH_LINK_MNG_ESTAB;
	PeerLinkEstablished(pAd, Idx);
	return;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get ClsAcpt event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_CLOSE_RCVD);
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNRjct event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNRjct event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOR2 event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_MAX_RETRIES);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	return;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Cncl event at OpnRcvd (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, MESH_LINK_CANCELLED);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNAcpt event at Estab (Idx=%d).\n", __func__, Idx));
	pMacEntry = MeshTableLookup(pAd, pAd->MeshTab.MeshLink[Idx].Entry.PeerMacAddr, TRUE);

	if (pMacEntry)
		EnquePeerLinkConfirm(pAd, Idx, pMacEntry->Aid, MLME_SUCCESS);
	else
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("%s: MacEntry doesn't exist.\n", __func__));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get ClsAcpt event at Estab (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_CLOSE_RCVD);
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNRjct event at Estab (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNRjct event at Estab (Idx=%d).\n", __func__, Idx));
	RTMPCancelTimer(&pAd->MeshTab.MeshLink[Idx].TOR, &Cancelled);
	EnquePeerLinkClose(pAd, Idx, pInfo->ReasonCode);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get Cncl event at Estab (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_LINK_CANCELLED);
	RTMPSetTimer(&pAd->MeshTab.MeshLink[Idx].TOH, TOH_TIME);
	*pCurrState = MESH_LINK_MNG_HOLDING;
	return;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get TOH event at Holding (Idx=%d).\n", __func__, Idx));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get OPNActp at Holding (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_LINK_CANCELLED);
	return;
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CFNActp at Holding (Idx=%d).\n", __func__, Idx));
	EnquePeerLinkClose(pAd, Idx, MESH_LINK_CANCELLED);
	return;
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: Get CLSActp at Holding (Idx=%d).\n", __func__, Idx));
	...
	IN UCHAR LinkIdx)
{
	...
	if (WMODE_CAP_N(wdev->PhyMode)) {
		...
		InsertAddHtInfoIE(pAd, (pOutBuffer + FrameLen), &FrameLen, addht);
	}
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId(%d)=%x\n",
			 __func__, LinkIdx, pAd->MeshTab.MeshLink[LinkIdx].Entry.LocalLinkId));
	return;
}
	...
	IN UINT16 StatusCode)
{
	...
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId(%d)=%x, PeerLinkId=%x\n",
			 __func__, LinkIdx, pAd->MeshTab.MeshLink[LinkIdx].Entry.LocalLinkId,
	...
	IN UINT16 ReasonCode)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId(%d)=%x, PeerLinkId=%x, Reason=%d\n",
			 __func__, LinkIdx, pAd->MeshTab.MeshLink[LinkIdx].Entry.LocalLinkId,
	...
	IN UINT16 ReasonCode)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId=%x, PeerLinkId=%x, Reason=%d\n",
			 __func__, LocalLinkId, PeerLinkId, ReasonCode));
	return;
}
	...
	IN UCHAR HtCapabilityLen)
{
	...
	if ((pAd->MeshTab.wdev.PhyMode == PHY_11G) && (MaxSupportedRate < RATE_FIRST_OFDM_RATE))
		return MLME_ASSOC_REJ_DATA_RATE;

#ifdef DOT11_N_SUPPORT

	/* 11n only */
	if (WMODE_HT_ONLY(pAd->MeshTab.wdev.PhyMode) && (HtCapabilityLen == 0))
		return MLME_ASSOC_REJ_DATA_RATE;
	...
	do {
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("MeshPeerOpen - MaxSupportedRate = %x, pAd->CommonCfg.MaxTxRate = %x\n",
		...
		if (pEntry->Aid == 0)
			...
			/* MacTableSetEntryRaCap(pAd, pEntry, ClientRalinkIe); */
#ifdef DOT11_N_SUPPORT

			/* If this Entry supports 802.11n, upgrade to HT rate. */
			if ((HtCapabilityLen != 0) &&
				(pAd->MeshTab.wdev.DesiredHtPhyInfo.bHtEnable) &&
				WMODE_CAP_N(pAd->MeshTab.wdev.PhyMode)) {
...
#endif /* CONFIG_STA_SUPPORT */
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshBuildOpen BW_%d %d %d\n",
				...
				if (pAd->MeshTab.wdev.DesiredTransmitSetting.field.MCS != MCS_AUTO) {
					MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_OFF, ("@@@ pAd->CommonCfg.RegTransmitSetting.field.MCS = %d\n",
	...
	IN RX_BLK * pRxBlk)
{
	...
	do {
		...
		MeshLinkMngOpenSanity(pAd,
								...
								&HTCapability,
								&HTCapability_Len);
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: PeerLinkId=%x, PeerMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				 __func__, PeerLinkId, pHeader->Addr2[0], pHeader->Addr2[1], pHeader->Addr2[2],
		...
		if (Idx == BSS_NOT_FOUND) {
			...
			if (Idx == BSS_NOT_FOUND) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() OpenRjct, PeerLink Not found.\n", __func__));
		...
		if (MeshTableLookup(pAd, pHeader->Addr2, TRUE)) {
			...
			if (PeerLinkId != pAd->MeshTab.MeshLink[Idx].Entry.PeerLinkId) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() OpenRjct, PeerLinkId not match. %x, %x\n",
						 __func__, PeerLinkId, pAd->MeshTab.MeshLink[Idx].Entry.PeerLinkId));
				result = MESH_LINK_MNG_OPEN_RJCT;
		...
		if (!(NdisEqualMemory(MeshId, pAd->MeshTab.MeshId, pAd->MeshTab.MeshIdLen)
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() OpenIgnr, Link Instance Not match.\n", __func__));
		...
		if (!MeshCapabilty.field.AcceptPeerLinks) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() OpenRjct, Not AcceptPeerLinks.\n", __func__));
			result = MESH_LINK_MNG_OPEN_RJCT;
			ReasonCode = MESH_MAX_PEERS;
			break;
		}
		...
		if (PeerMsaIeLen < sizeof(MSA_HANDSHAKE_IE)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The received length of MSAIE is too short(%d)\n", PeerMsaIeLen));
			result = MESH_LINK_MNG_OPEN_RJCT;
		...
		if (PeerMscIe.MeshSecurityConfig.field.DefaultRole !=
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Default Role Negotiation doesn't match(%d)\n", PeerMscIe.MeshSecurityConfig.field.DefaultRole));
			result = MESH_LINK_MNG_OPEN_RJCT;
		...
		ReasonCode = MeshCheckPeerMsaIeCipherValidity(pAd, SUBTYPE_PEER_LINK_OPEN,
						  pMeshLinkEntry, PeerMsaIe);
		if (ReasonCode != MLME_SUCCESS) {
			result = MESH_LINK_MNG_OPEN_RJCT;
			break;
		}
		...
		if (!MeshKeySelectionAction(pAd, pMeshLinkEntry, (PUCHAR)&PeerMscIe, &RSN_IE[RSNIE_Len - pmkid_len], pmkid_len)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("No any MP connect to MKD !!!\n"));
			result = MESH_LINK_MNG_OPEN_RJCT;
		...
		if (PeerLinkCfnRcvd(pAd, Idx)) {
			ReasonCode = MeshValidateOpenAndCfnPeerLinkMsg(pAd,
				SUBTYPE_PEER_LINK_OPEN, pMeshLinkEntry, (PUCHAR)&PeerMscIe, PeerMsaIe,
				PeerMsaIeLen, RSN_IE, RSNIE_Len, pure_rsn_len, pmkid_len);
	...
	IN RX_BLK * pRxBlk)
{
	...
	do {
		...
		MeshLinkMngCfnSanity(pAd,
								...
								&HTCapability,
								&HTCapability_Len);
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId=%x, PeerLinkId=%x, PeerMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				 __func__, LocalLinkId, PeerLinkId, pHeader->Addr2[0], pHeader->Addr2[1],
				 pHeader->Addr2[2], pHeader->Addr2[3], pHeader->Addr2[4], pHeader->Addr2[5]));

		if (!(NdisEqualMemory(MeshId, pAd->MeshTab.MeshId, pAd->MeshTab.MeshIdLen)
			  && (MeshSelPathId == pAd->MeshTab.PathProtocolId)
			  && (MeshSelMetricId == pAd->MeshTab.PathMetricId))) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() CfnIgnr, Link Instance Not match.\n", __func__));
			result = MESH_LINK_MNG_CFN_IGNR;
			ReasonCode = MLME_UNSPECIFY_FAIL;
			break;
		}
		...
		if (StatusCode != MLME_SUCCESS) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() CfnRjct, Status Code=%d.\n", __func__, StatusCode));
		...
		if (PeerLinkId != pAd->MeshTab.MeshLink[MeshLinkId].Entry.LocalLinkId) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() CfnRjct, Link Id Not match. %x, %x\n",
					 __func__, PeerLinkId, pAd->MeshTab.MeshLink[MeshLinkId].Entry.LocalLinkId));
			result = MESH_LINK_MNG_CFN_RJCT;
			ReasonCode = MLME_UNSPECIFY_FAIL;
			break;
		}
		...
		/* Verify the length of the received MSAIE */
		if (PeerMsaIeLen < sizeof(MSA_HANDSHAKE_IE)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The received length of MSAIE in P.L. confirm frame is too short(%d)\n", PeerMsaIeLen));
			result = MESH_LINK_MNG_CFN_RJCT;
			ReasonCode = MESH_SECURITY_FAILED_VERIFICATION;
			break;
		}
		...
		if (PeerLinkOpnRcvd(pAd, Idx)) {
			ReasonCode = MeshValidateOpenAndCfnPeerLinkMsg(pAd,
				SUBTYPE_PEER_LINK_CONFIRM, pMeshLinkEntry, (PUCHAR)&PeerMscIe,
				PeerMsaIe, PeerMsaIeLen, RSN_IE, RSNIE_Len, pure_rsn_len, pmkid_len);
		...
		/* On the other hand, if the local MP has not received a peer link open frame from the candidate peer MP, */
			...
			ReasonCode = MeshCheckPeerMsaIeCipherValidity(pAd,
				  SUBTYPE_PEER_LINK_CONFIRM, pMeshLinkEntry, PeerMsaIe);
			if (ReasonCode != MLME_SUCCESS) {
				result = MESH_LINK_MNG_OPEN_RJCT;
				break;
			}
	...
	IN RX_BLK * pRxBlk)
{
	...
	do {
		...
		if (Idx == BSS_NOT_FOUND) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() ClsIgnore, PeerLink Not found.\n", __func__));
		...
		MeshLinkMngClsSanity(pAd,
								...
								&PeerLinkId,
								&ReasonCode);
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s: LocalLinkId=%x, PeerLinkId=%x, Reason=%d, PeerMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				 __func__, LocalLinkId, PeerLinkId, ReasonCode, pHeader->Addr2[0], pHeader->Addr2[1],
				 pHeader->Addr2[2], pHeader->Addr2[3], pHeader->Addr2[4], pHeader->Addr2[5]));

		if ((PeerLinkId != pAd->MeshTab.MeshLink[MeshLinkId].Entry.LocalLinkId)
			|| (LocalLinkId != pAd->MeshTab.MeshLink[MeshLinkId].Entry.PeerLinkId)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("%s() ClsIgnr, Link Id Not match.\n", __func__));
			result = MESH_LINK_MNG_CLS_IGNR;
			ReasonCode = MLME_UNSPECIFY_FAIL;
			break;
		}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (idx = 0; idx < MAX_MESH_LINKS; idx++) {
		...
		if (RTMP_TIME_AFTER(Now, pAd->MeshTab.MeshLink[idx].Entry.LastBeaconTime + (MESH_AGEOUT_TIME * OS_HZ / 1000))) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("ageout MESH #%d.\n", idx));
