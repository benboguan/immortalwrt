	...
	OUT PMESH_CONFIGURAION_IE pMeshConfiguration)
{
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		default:
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("MeshPeerBeaconAndProbeSanity - unrecognized EID = %d\n", pEid->Eid));
	...
	while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((PUCHAR)pMsg + MsgLen)) {
		...
		case IE_SUPP_RATES:
			if ((eid_ptr->Len <= MAX_LEN_OF_SUPPORTED_RATES) && (eid_ptr->Len > 0)) {
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("%s - IE_SUPP_RATES., Len=%d. Rates[0]=%x\n", __func__, eid_ptr->Len, SupRate[0]));
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("SupRate[1]=%x %x %x %x %x %x %x\n",
				...
				*pSupRateLen = eid_ptr->Len;
				...
				/*MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("PeerAssocReqSanity - wrong IE_SUPP_RATES\n")); */
				*pSupRateLen = 8;
				SupRate[0] = 0x82;
				SupRate[1] = 0x84;
				SupRate[2] = 0x8b;
				...
				SupRate[7] = 0x6c;
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_INFO, ("PeerAssocReqSanity - wrong IE_SUPP_RATES., Len=%d\n", eid_ptr->Len));
#endif /* RELEASE_EXCLUDE */
				/*return FALSE; */
			}
		...
		case IE_WPA2:
			...
			if (pAd->MeshTab.wdev.AuthMode < Ndis802_11AuthModeWPA) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("The AuthMode isn't WPA!!!%d\n", pAd->MeshTab.wdev.AuthMode));
				break;
			}
			...
			if (!NdisEqualMemory(eid_ptr->Octet, WPA1_OUI, sizeof(WPA1_OUI))
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Not RSN IE, maybe proprietary IE!!!\n"));
				break;
			}
			...
			if (/*(eid_ptr->Len <= MAX_LEN_OF_RSNIE) && */(eid_ptr->Len > MIN_LEN_OF_RSNIE)) {
				...
				*pRsnIeLen = 0;
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("The received the length(%d) of RSN_IE is invalid\n", eid_ptr->Len));
				break;
			}

			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("Link Msg - RSNIE(%d), and its len(%d)!!!\n", eid_ptr->Eid, *pRsnIeLen));
			break;
		...
		case IE_HT_CAP:
			if (eid_ptr->Len >= sizeof(HT_CAPABILITY_IE)) {
				...
				*pHtCapabilityLen = SIZE_HT_CAP_IE;
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_WARN, ("PeerAssocReqSanity - IE_HT_CAP\n"));
			} else
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_WARN, ("PeerAssocReqSanity - wrong IE_HT_CAP.eid_ptr->Len = %d\n", eid_ptr->Len));
	...
	OUT	UCHAR			*PmkIdLen)
{
	...
	if (pAd->MeshTab.wdev.AuthMode < Ndis802_11AuthModeWPA) {
		if (peerRsnIeLen == 0) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the local MP has no RSNIE\n"));
			return FALSE;
		}
	} else {
		if (peerRsnIeLen == 0) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the peer MP has no RSNIE\n"));
			return FALSE;
		}
	}
	...
	if (eid_ptr->Eid != IE_WPA && eid_ptr->Eid != IE_WPA2) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE has unknown element ID(%d)\n", eid_ptr->Eid));
		return FALSE;
	}

	if (eid_ptr->Len < sizeof(RSNIE2)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, its length is too short(%d)\n", eid_ptr->Len));
		return FALSE;
	}
	...
	*PmkIdLen = 0;
	...
	if (MeshCheckGroupCipher(pAd, pBufTmp + offset, RsnEid)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the group cipher isn't supported\n"));
		return FALSE;
	}
	...
	if (Count > 0) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the count of piarwise-cipher is invalid(%d)\n", Count));
		return FALSE;
	}
	...
	if (!isValid) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the pairwise cipher isn't supported\n"));
		return FALSE;
	}
	...
	if (Count > 0) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the count of AKM-suite is invalid(%d)\n", Count));
		return FALSE;
	}
	...
	if (!isValid) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("MeshValidateRSNIE, the AKM suite isn't supported\n"));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshValidateRSNIE is completed\n"));
	...
	*PureRsnLen = offset;	/* the length is the RSNIE except elementID, length and PMKID */
	...
	if (eid_ptr->Len - offset >= 2) {
		...
		if (*PmkIdLen > 0)
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_TRACE, ("MeshValidateRSNIE, %d PMKID exist and its len is %d\n", Count, (eid_ptr->Len - offset)));
	}
	...
	IN UCHAR				pmkid_len)
{
	...
	if (!RTMPEqualMemory(pRcvdMscIe, (PUCHAR)&pMeshLinkEntry->RcvdMscIe, sizeof(MESH_SECURITY_CAPABILITY_IE))) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The MSCIE is different between peer link open and confirm frame\n"));
	...
	if (!RTMPEqualMemory(pRcvdMsaIe, pMeshLinkEntry->RcvdMsaIe, sizeof(MESH_HANDSHAKE_CONTROL))) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Handshake Control field of MSAIE is different between peer link open and confirm frame\n"));
		return MESH_SECURITY_FAILED_VERIFICATION;
	}
	...
	if (pAd->MeshTab.wdev.AuthMode >= Ndis802_11AuthModeWPA) {
		...
		if (pure_rsn_len != pMeshLinkEntry->RcvdRSNIE_Len ||
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The RSNIE is different between candidate MP's link open and confirm frame\n"));
			return MESH_SECURITY_FAILED_VERIFICATION;
		}
		...
		if (pmkid_len > 0 && pMeshLinkEntry->RcvdPMKID_Len > 0) {
			...
			if (pmkid_len != pMeshLinkEntry->RcvdPMKID_Len ||
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The result of Key Selection is different between candidate MP's link open and confirm frame\n"));
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The PMKID is different between candidate MP's link open and confirm frame\n"));
			return MESH_SECURITY_FAILED_VERIFICATION;
		}
		...
		if (!pMeshLinkEntry->bValidLocalMpAsSelector) {
			...
			if (!RTMPEqualMemory(pRcvdMsaIe + akm_offset, &pMeshLinkEntry->RcvdMsaIe[akm_offset], LEN_OUI_SUITE)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected AKM Suite of MSAIE is different between peer link open and confirm frame\n"));
				return MESH_SECURITY_FAILED_VERIFICATION;
			}

			if (!RTMPEqualMemory(pRcvdMsaIe + cipher_offset, &pMeshLinkEntry->RcvdMsaIe[cipher_offset], LEN_OUI_SUITE)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected Pairwise Cipher Suite of MSAIE is different between peer link open and confirm frame\n"));
				return MESH_SECURITY_FAILED_VERIFICATION;
			}
		}
	}
...
#endif /* RELEASE_EXCLUDE */
	{
		...
		if (!NdisEqualMemory(LocalMaAddr, PeerMaAddr, MAC_ADDR_LEN)) {
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The MA-ID of MSAIE doesn't match\n"));
			return MESH_SECURITY_FAILED_VERIFICATION;
		}
	}
	return MLME_SUCCESS;
	...
	if (pAd->MeshTab.wdev.AuthMode >= Ndis802_11AuthModeWPA) {
		...
		if (((state == SUBTYPE_PEER_LINK_OPEN) && (!pMeshLinkEntry->bValidLocalMpAsSelector)) ||
			...
			if (!MeshCheckPairwiseCipher(pAd, &pRcvdMsaIe[cipher_offset], RsnEid)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected Pairwise Cipher isn't supported by local MP\n"));
				return MESH_SECURITY_ROLE_NEGOTIATION_DIFFERS;
			}

			if (!MeshCheckAKMSuite(pAd, &pRcvdMsaIe[akm_offset], RsnEid)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected AKM isn't supported by local MP\n"));
				return MESH_SECURITY_ROLE_NEGOTIATION_DIFFERS;
			}
		...
		if ((state == SUBTYPE_PEER_LINK_CONFIRM) && (pMeshLinkEntry->bValidLocalMpAsSelector)) {
			if (!NdisEqualMemory(&pRcvdMsaIe[akm_offset], &pMeshLinkEntry->LocalMsaIe[akm_offset], LEN_OUI_SUITE)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected AKM in P.L. confirm frame isn't supported by local Selector MP\n"));
				return MESH_SECURITY_FAILED_VERIFICATION;
			}

			if (!NdisEqualMemory(&pRcvdMsaIe[cipher_offset], &pMeshLinkEntry->LocalMsaIe[cipher_offset], LEN_OUI_SUITE)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_MESH, DBG_LVL_ERROR, ("The Selected Pairwise Cipher in P.L. confirm frame isn't supported by local Selector MP\n"));
				return MESH_SECURITY_FAILED_VERIFICATION;
			}
		}
	}
