...
drv_mtk_init_device_config() {
	...
	config_add_string tx_burst cell_density
...
drv_mtk_init_iface_config() {
	...
	config_add_string mode ifname 'macaddr:macaddr' 'bssid:macaddr' 'ssid:string' encryption
...
mtk_ap_vif_pre_config() {
	...
	ApBcnReq="${ApBcnReq}${rrm_beacon_report:-1};"
...
mtk_mesh_vif_pre_config() {
	...
	# mt_cmd iwpriv ra${MTWIFI_IFPREFIX}0 set MeshAutoLink=1
...
drv_mtk_setup() {
	...
	json_get_vars main_if phy_name macaddr channel mode hwmode htmode \
		...
		diversity frag rts txburst hidden \
	...
	case "$phy_name" in
		ra0)
			...
			MTWIFI_IFPREFIX=""
			MTWIFI_DEF_BAND="g"
			MTWIFI_PROFILE_PATH="${MTWIFI_PROFILE_DIR}mt7615.1.2G.dat"
			MTWIFI_CMD_PATH="${MTWIFI_PROFILE_DIR}mt7615.1.2G.sh"
			MTWIFI_CMD_OPATH="${MTWIFI_PROFILE_DIR}mt7615.1.5G.sh"
		;;
		rax0)
			WirelessMode=14
			APCLI_IF="apclix0"
			...
			MTWIFI_IFPREFIX="x"
			MTWIFI_DEF_BAND="a"
			MTWIFI_PROFILE_PATH="${MTWIFI_PROFILE_DIR}mt7615.1.5G.dat"
			MTWIFI_CMD_PATH="${MTWIFI_PROFILE_DIR}mt7615.1.5G.sh"
			MTWIFI_CMD_OPATH="${MTWIFI_PROFILE_DIR}mt7615.1.2G.sh"
		;;
		*)
...
#HT/VHT默认模式设定
	...
	case "$htmode" in
		HT20)
			[ "$hwmode" == "a" ] && WirelessMode=8 || WirelessMode=9
			HT_BW=0
			VHT_BW=0
		;;
		HT40)
...
CountryRegionABand=${countryregion_a}
CP_SUPPORT=2
CSPeriod=6
DataFloodThreshold=64
DBDC_MODE=1
DbdcBandSupport=0
...
VHT_BW=${VHT_BW}
VHT_BW_SIGNAL=0
VHT_DisallowNonVHT=${VhtDisallowNonVHT:-0}
VHT_LDPC=${ldpc}
VHT_Sec80_Channel=0
VHT_SGI=${VHT_SGI}
VHT_STBC=${tx_stbc}
...
#For DBDC profile merging......
	...
	echo "BcnReq=${ApBcnReq%?}" >> $MTWIFI_PROFILE_PATH
