...
detect_mtk() {
	local macaddr htbsscoex ifname
			...
			[ "$type" == "mtk" ] || {
				case $phyname in
					ra0)
						...
						htbsscoex="1"
#						ifname="ra0"
						[ -n "$(is_11ax_dbdc_dev)" ] && htmode=HE40 || htmode=VHT40
						[ -z "$hostname" ] && {
					...
					rax0)
						...
						htbsscoex="0"
#						ifname="rax0"
						[ -n "$(is_support_11ax_ht160_dev)" ] && htmode=HE160 || {
...
#				}
				uci -q batch <<-EOF
					...
					set wireless.${phyname}.noscan=$htbsscoex
